...
int broj; /*ovdje spremam broj koji na početku pročitam iz statusa*/
int obrada_broj = 0;
int nije_kraj = 1;
FILE *fileStatus;
FILE *fileObrada;
...
struct sigaction act;
 ...
 printf("Program s PID=%ld krenuo s radom\n", (long) getpid());
    ...
    {
        /*ako u statusu nije 0, tj. ako nije došlo do prekida*/
        obrada_broj = broj;
    }
    fclose(fileObrada);
...
{ 
    ...
    fclose(fileStatus);    
    printf("Primio signal SIGTERM, pospremam %d prije izlaska iz programa\n", obrada_broj);    
    exit(1);
}
